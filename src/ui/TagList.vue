<template>
  <div v-show="getItems().length" class="flex px-8 py-5">
    <div v-for="item in getItems()" :key="getItemId(item)" class="flex items-center border 
                                                  bg-gray-300 
    ...
    <button v-show="getItems().length > 1" class="button" @click="clearAll()">
      Clear all
    </button>
  </div>
</template>
